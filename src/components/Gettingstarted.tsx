...
// API response shape from /api/resources
...
export default function GettingStartedComponent() {
  ...
  // ── Step 1: Fetch location options for the dropdown ───────────────────────
  useEffect(() => {
    const fetchMeta = async () => {
      try {
        const res = await fetch(`/api/resources?scope=INDIA_STATE&meta=1`);
        if (!res.ok) throw new Error(`API error: ${res.status}`);
        const data: { locations: LocationOption[] } = await res.json();
        setLocationOptions(data.locations);

        // Default-select "India" / "National" if present, else first option
        const defaultKey =
          data.locations.find((l) => l.locationKey === "India")?.locationKey ??
          data.locations[0]?.locationKey ??
          "";
        if (defaultKey) setSelectedKeys([defaultKey]);
      } catch (err) {
    ...
    const fetchSchemes = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/resources?scope=INDIA_STATE&locationKeys=${encodeURIComponent(keys)}`
        );
        if (!res.ok) throw new Error(`API error: ${res.status}`);
  ...
  // ── Flatten grouped → flat list for rendering ────────────────────────────
  const currentSchemes: Scheme[] = Object.values(grouped).flatMap((g) => g.schemes);

  // ── Dropdown display values (locationLabel) ───────────────────────────────
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
            <div>
              ...
              <p className="text-muted-foreground max-w-3xl">
                ...
                !selectedKeys.includes("India") &&
                !selectedKeys.includes("central-government")
              ...
              {currentSchemes.map((scheme) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Expanded: full description + learn more link */}
