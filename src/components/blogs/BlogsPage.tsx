...
type ApiResponse = {
  ...
  page: number;
  limit: number;
  // bundled meta — always present, no second fetch needed
...
type SearchSuggestion = {
  ...
  date: string; slug: string; relevance: number;
};
...
function matchesReadingTimeBucket(minutes: number | null, bucket: string): boolean {
  if (!minutes) return false;
  if (bucket === "Under 5 min") return minutes <= 5;
...
// ✅ Single URL builder — contentType always BLOG, meta bundled in main response
...
export default function BlogsPage() {
    ...
    setBlogItems(
      selectedReadingTimes.length === 0
        ...
        : allItems.filter((item) =>
            selectedReadingTimes.some((b) => matchesReadingTimeBucket(item.readingTime, b))
          )
    );
  }, [allItems, selectedReadingTimes]);

  // ── On mount: ONE fetch returns blogs + meta ────────────────────────────────
  // ✅ Was: Promise.all([fetch(blogs), fetch(meta)]) = 2 HTTP requests, 2 DB round-trips
  // Now:   single fetch, API bundles meta (from cache after first hit) in response
  useEffect(() => {
    (async () => {
      try {
        ...
        // Meta comes bundled — no second fetch
  ...
  const fetchFilteredBlogs = useCallback(async () => {
    ...
    try {
      ...
      setTotalPages(data.totalPages);
      setTotalItems(data.totalItems);
      // Update meta if server returned fresher data (e.g. new category added)
  ...
  // ── Search suggestions ──────────────────────────────────────────────────────
  // ✅ Uses /api/content/search — scans ALL ~1900 rows, not just page 1
  // Articles on page 73 show up in suggestions just like page 1 articles
  useEffect(() => {
    ...
    (async () => {
      try {
        const params = new URLSearchParams({ q: debouncedSearch, contentType: "BLOG" });
        ...
        // Rank the 50 server candidates by relevance, keep top 8
        const suggestions: SearchSuggestion[] = results
          .map((r) => {
            let relevance = 0;
            const titleLower = r.title.toLowerCase();
            if (titleLower.startsWith(q))   relevance += 15; // strongest: starts with query
            if (titleLower.includes(q))     relevance += 10; // title contains query
  ...
  const clearAllFilters = () => {
    setSelectedCategorySlug(""); setSelectedTagSlug(""); setSelectedReadingTimes([]);
    setDateRange({ from: "", to: "" }); setSelectedDateRange(""); setShowCustomDatePicker(false);
    setSearchQuery(""); setDebouncedSearch(""); setSearchSuggestions([]);
    setShowSuggestions(false); setCurrentPage(1); setIsFilterOpen(false);
  };
                  ...
                  <div className="p-4 sm:p-6">
                    <div className="flex items-center justify-between mb-2">
                      <span className="inline-flex items-center gap-1 px-2 sm:px-3 rounded-full bg-primary/10 text-primary text-xs font-semibold uppercase">
                        {getCategoryIcon(featuredBlog.categoryName ?? "")}
                        {featuredBlog.categoryName ?? "Blog"}
                      </span>
                              ...
                              <div className="flex-shrink-0 relative h-12 w-12 sm:h-14 sm:w-14 rounded-lg overflow-hidden bg-gradient-to-br from-muted to-secondary">
                                {blog.featuredImage ? (
                                  ...
                                  <div className="absolute inset-0 bg-gradient-to-r from-primary/20 to-accent/20 flex items-center justify-center">
                                    <span className="text-primary/40 text-lg font-display">{blog.title.charAt(0)}</span>
                                  </div>
                                )}
                        ...
                        <Link href={`/blogs/${blog.slug}`}
                          ...
                          <div className="relative h-40 sm:h-44 bg-gradient-to-br from-muted to-secondary flex-shrink-0 overflow-hidden">
                            {blog.featuredImage ? (
                              ...
                              <div className="absolute inset-0 bg-gradient-to-r from-primary to-accent flex items-center justify-center">
                                <div className="text-white/40 text-5xl font-display">{blog.title.charAt(0)}</div>
                              </div>
                            )}
                          </div>
                          <div className="p-4 sm:p-6 flex flex-col flex-grow">
                            <div className="flex items-center justify-between mb-2 sm:mb-3">
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full bg-accent/10 text-accent text-xs font-semibold uppercase">
                                {getCategoryIcon(blog.categoryName ?? "")}
                                {(blog.categoryName ?? "Blog").split(" & ")[0]}
                              </span>
                              {blog.readingTime && (
                                <span className="text-xs text-muted-foreground flex items-center gap-1">
                            ...
                            {blog.tags.length > 0 && (
                              <div className="flex flex-wrap gap-1 mb-3">
                                {blog.tags.slice(0, 3).map((tag) => (
                                  <button key={tag.id}
                                    onClick={(e) => { e.preventDefault(); setSelectedTagSlug(tag.slug); setCurrentPage(1); }}
