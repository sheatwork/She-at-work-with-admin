...
export default function EntreChatPage() {
  ...
  // ── Data state ──────────────────────────────────────────────────────────────
  const [allItems, setAllItems] = useState<EntreChatItem[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const params = new URLSearchParams({ q: debouncedSearch, contentType: "ENTRECHAT" });
                      ...
                      >
                            ...
                            <div className="flex flex-wrap gap-2">
                              {industrySectors.map((sector) => (
                                <button
                                  key={sector}
                                  onClick={() => {
                                    setSelectedIndustrySector(selectedIndustrySector === sector ? "" : sector);
                                    setCurrentPage(1);
                                  }}
                                  className={`px-3 py-1.5 text-xs rounded-full border transition-all ${
                                    selectedIndustrySector === sector
                            ...
                            <div className="flex flex-wrap gap-2">
                              {businessStages.map((stage) => (
                                <button
                                  key={stage}
                                  onClick={() => {
                                    setSelectedBusinessStage(selectedBusinessStage === stage ? "" : stage);
                                    setCurrentPage(1);
                                  }}
                                  className={`px-3 py-1.5 text-xs rounded-full border transition-all ${
                                    selectedBusinessStage === stage
                                      ? "bg-primary text-white border-primary"
                                      : "bg-secondary/50 border-border hover:bg-secondary"
                                  }`}
                                >
                            ...
                            <div className="flex flex-wrap gap-2">
                              {interviewFormats.map((format) => (
                                <button
                                  key={format}
                                  onClick={() => {
                                    setSelectedInterviewFormat(selectedInterviewFormat === format ? "" : format);
                                    setCurrentPage(1);
                                  }}
                                  className={`px-3 py-1.5 text-xs rounded-full border transition-all ${
                                    selectedInterviewFormat === format
                                      ? "bg-primary text-white border-primary"
                                      : "bg-secondary/50 border-border hover:bg-secondary"
                                  }`}
                                >
                            ...
                            <div className="flex flex-wrap gap-2">
                              {founderRegions.map((region) => (
                                <button
                                  key={region}
                                  onClick={() => {
                                    setSelectedFounderRegion(selectedFounderRegion === region ? "" : region);
                                    setCurrentPage(1);
                                  }}
                                  className={`px-3 py-1.5 text-xs rounded-full border transition-all ${
                                    selectedFounderRegion === region
                                      ? "bg-primary text-white border-primary"
                                      : "bg-secondary/50 border-border hover:bg-secondary"
                                  }`}
                                >
                            ...
                            <div className="flex flex-wrap gap-2">
                              {successFactors.map((factor) => (
                                <button
                                  key={factor}
                                  onClick={() => {
                                    setSelectedSuccessFactor(selectedSuccessFactor === factor ? "" : factor);
                                    setCurrentPage(1);
                                  }}
                                  className={`px-3 py-1.5 text-xs rounded-full border transition-all ${
                                    selectedSuccessFactor === factor
                                      ? "bg-primary text-white border-primary"
                                      : "bg-secondary/50 border-border hover:bg-secondary"
                                  }`}
                                >
                        ...
                        <div className="p-3 bg-secondary/30 rounded-lg mb-4">
                          ...
                          {userLocation.detected ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              Click detect to filter by your location
                            </p>
                          )}
                        </div>
