...
import { db } from "@/db";
import { StorySubmissionsTable, UsersTable, ContentTable, CategoriesTable } from "@/db/schema";
...
export async function GET(_req: NextRequest, { params }: Context) {
  ...
  try {
    ...
    // If already published, fetch the linked content item for context
    ...
    return NextResponse.json({ success: true, data: { ...row, publishedContent } });
  } catch (err) {
...
// Used for simple status changes: PENDING → REJECTED, or updating reviewNotes.
